(()=>{
  ...
  function inject(){
    ...
    manager.onclick=()=>{originalDashboard?.click();closeMobile()};
    ...
    basic.onclick=()=>requirePlaylist(()=>{document.querySelector('.channel-panel')?.scrollIntoView({behavior:'smooth',block:'start'});flash(document.querySelector('.channel-panel'));closeMobile()});
    ...
    manual.onclick=()=>requirePlaylist(()=>{$('refreshSource')?.click();closeMobile()});
    ...
    users.onclick=()=>{const a=$('adminNav')||adminNav;if(a&&!a.classList.contains('hidden')){a.click();closeMobile()}else toast('Admin access required')};
